...
export const MemoryReviewSection: React.FC<MemoryReviewSectionProps> = ({
  ...
  return (
    <div id="memory-review-section" className="space-y-6 pt-6 pb-12 font-sans animate-fade-in">
      {/* Section Header */}
      <div className="bg-stone-900/60 border border-stone-800/90 rounded-2xl p-5 space-y-2">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2.5">
            ...
            <div>
              <h2 className="text-base font-serif font-medium text-stone-100">
                Worth remembering?
              </h2>
          ...
          {savedMemories.length > 0 && (
            <div className="hidden sm:flex items-center space-x-1.5 px-3 py-1 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-medium">
      ...
      {/* Loading State */}
      {loading && (
        <div className="py-12 px-6 rounded-2xl bg-stone-900/30 border border-stone-800/80 text-center space-y-3">
      ...
      {!loading && error && (
        <div className="p-4 rounded-xl bg-red-950/20 border border-red-900/30 space-y-3">
          <div className="flex items-start space-x-2 text-xs text-red-300">
            <AlertCircle className="w-4 h-4 shrink-0 mt-0.5 text-red-400" />
            <span>{error}</span>
          </div>
          <button
            onClick={loadSuggestions}
            className="inline-flex items-center space-x-1.5 px-3 py-1.5 rounded-lg bg-stone-800 hover:bg-stone-700 text-stone-200 text-xs font-medium transition cursor-pointer"
          >
      ...
      {!loading && !error && candidates.length === 0 && (
        <div className="py-10 px-6 rounded-2xl bg-stone-900/20 border border-stone-800 text-center space-y-2">
          <div className="w-8 h-8 rounded-full bg-stone-800/60 flex items-center justify-center mx-auto text-stone-400">
      ...
      {savedMemories.length > 0 && (
        <div className="pt-2">
          <div className="p-4 rounded-xl bg-stone-950/80 border border-stone-800 space-y-2">
            ...
            <ul className="space-y-1.5 pl-6 list-disc text-xs text-stone-400 font-serif">
              {savedMemories.map((m) => (
                <li key={m.id} className="leading-relaxed">
                  <span className="text-stone-300 font-sans font-medium text-[11px] capitalize mr-1">
      ...
      <div className="pt-4 border-t border-stone-800 flex items-center justify-between">
        ...
        <button
          id="done-review-btn"
          onClick={onDone}
          className="inline-flex items-center space-x-2 px-5 py-2.5 rounded-xl bg-stone-100 hover:bg-white text-stone-950 text-xs font-semibold shadow transition cursor-pointer"
        >
